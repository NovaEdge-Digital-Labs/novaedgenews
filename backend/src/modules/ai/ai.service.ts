...
export class AIService {
    ...
    public primaryClient: OpenAI;
    public fallbackClient: OpenAI;

    constructor(private readonly configService: ConfigService) {
        const openrouterKey = this.configService.get<string>('ai.openrouter');
        const geminiKey = this.configService.get<string>('ai.gemini');
    ...
    async rewriteArticle(rawTitle: string, rawContent: string): Promise<any> {
        ...
        try {
            ...
            try {
                ...
                // Clean Fallback (No robotic or placeholder text)
                return {
                    title: rawTitle,
    ...
    async rewriteAsQuickNews(rawTitle: string, rawContent: string): Promise<any> {
    ...
    async generateDeepAnalysis(quickSummary: string, originalContent: string): Promise<any> {
    ...
    async generateAnalysis(rawTitle: string, rawContent: string): Promise<any> {
    ...
    private validateAndFilter(result: any, isAnalysis: boolean, isQuickNews = false, isDeepAnalysis = false): any {
        ...
        if (isDeepAnalysis) {
            ...
            for (const section of sectionsList) {
                ...
                const count = text.split(/\s+/).filter((w: string) => w.length > 0).length;
                totalWords += count;
                if (count === 0) throw new Error(`Deep Analysis rejected: Section "${section}" is empty`);
            }
    ...
    public async callAI(client: OpenAI, prompt: string, model: string): Promise<any> {
        if (!client) {
        ...
        // Flexible validation of core fields
        ...
        // Auto-map headlineA/B if missing but headline exists
        ...
        if (!result.headlineB && result.headlineA) {
            result.headlineB = { ...result.headlineA, type: 'curiosity' }; // Fallback
        }
